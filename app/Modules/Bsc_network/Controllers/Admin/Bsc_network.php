<?php 
namespace App\Modules\Bsc_network\Controllers\Admin;

class Bsc_network extends BaseController {
	...
	{    
	    ...
	    $existEthTest 	= false;
	  	...
	  	foreach ($existNetwork as $key => $exist) {
	  		if($exist->network_slug == 'bsc'){
	  			$existEth = true;
	  		}
	  		else if($exist->network_slug == 'bsc-testnet'){
	  			$existEthTest 	= true;
	  		}
	  	}
	    ...
	    $data['networks'] = $this->networks_model->where_rows('bsc_network', [], 'id', 'desc'); 
