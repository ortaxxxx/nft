...
class Verification
{
    ...
    { 
        ...
        if($result['status'] === TRUE){
            ...
            $str_whitelist = implode('-', $result['whitelist']);
            ...
            set_purchase_data('flag','purchase_key_used',true);
    ...
    private function serverAliveOrNot()
    {
        if($pf = @fsockopen($this->api_domain, 443)) {
            ...
            set_purchase_data('flag','serverAliveOrNot',false);
            return false;
        }
    }
    ...
    public function launch_application($data = [])
    {
        ...
        if($result['status'] === TRUE){
            ...
            if(!empty(get_purchase_data('flag','whitelist'))){
                $path = '../system/Security/lic.php'; 
                if (file_exists($path)) {
                    // Open the file
                    $whitefile = file_get_contents($path);
                    $str = get_purchase_data('flag','whitelist');
                    //set license key configuration
                    $new  = str_replace("{license_key}",@$str, $whitefile);
                    $new  = str_replace("{product_key}",@get_purchase_data('flag','product_key'), $new);
